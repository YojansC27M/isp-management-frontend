...
export const getReportMetrics = async (filters: ReportsFiltersValues) => {
  const { data } = await api.get<ReportMetrics>("/reports/metrics", { params: filters })
  return data
}

export const getRevenueData = async (filters: ReportsFiltersValues) => {
  const { data } = await api.get<RevenueData[]>("/reports/revenue", { params: filters })
  return data
}

export const getStatusDistribution = async (filters: ReportsFiltersValues) => {
  const { data } = await api.get<StatusDistribution[]>("/reports/status", { params: filters })
  return data
}

export const getOverdueClients = async (filters: ReportsFiltersValues) => {
  const { data } = await api.get<OverdueClient[]>("/reports/overdue", { params: filters })
  return data
}
