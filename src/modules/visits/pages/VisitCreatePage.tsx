...
interface VisitCreateLocationState {
  clientId?: string
  clientName?: string
}
...
const VisitCreatePage = () => {
  ...
  const { t } = useI18n()
  const { notify } = useUI()
  const state = location.state as VisitCreateLocationState | null

  const formInitialValues = useMemo<VisitFormValues>(
    () => ({
      ...initialValues,
  ...
  return (
    <div className="grid gap-6">
      ...
      <VisitForm initialValues={formInitialValues} onSubmit={handleSubmit} submitLabel={t("visits.create")} />
    </div>
  )
}
