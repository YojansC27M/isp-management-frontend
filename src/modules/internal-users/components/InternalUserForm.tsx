...
const InternalUserForm = ({ initialValues, onSubmit, submitLabel = "Guardar" }: InternalUserFormProps) => {
  ...
  const handleChange = (field: "name" | "email" | "phone" | "coverageZones" | "skills") => {
    return (event: ChangeEvent<HTMLInputElement>) => {
      setValues((current) => ({ ...current, [field]: event.target.value }))
    }
  }

  const handleSelect = (field: "role" | "status") => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(nextErrors).length > 0) {
      const order: FocusableField[] = ["name", "email", "phone", "role", "status", "coverageZones", "skills", "availabilityIds"]
